...
% \begin{frame}
%   {Faustregel der segmentalen Phonologie\\
%     \rot{mit ganz vielen Einschränkungen}}
%   \pause
%    \alert{Die graphematischen Formen bilden ungefähr
%     die zugrundeliegenden Formen ab, die Aussprache
%     entspricht den phonetischen Oberflächenformen.}\\
%   \pause
%   \Zeile
%   \normalsize\rot{Aber:}
%   \begin{enumerate}[<+->]
%     \item Das gilt nur unter Berücksichtigung der konkreten Segment"=Graph"=Korrelationen (/ʃ/ $\leftrightarrow$ \Lab sch\Rab usw.).
%     \item Gilt vor allem für \alert{Endrand-Desonorisierung}, \alert{/g/-Spirantisierung}, \alert{/ç/-Realisierung}, \alert{/ʁ/-Vokalisierung}, \alert{/ng/}
%     \item \rot{Gespanntheit} und \rot{Länge} werden graphematisch unzuverlässig kodiert.
%     \item \rot{[ʔ]} wird nicht graphematisch kodiert.
%     \item Graphematische \rot{Doppelkonsonanten} sind gesondert zu behandeln.
%     \item \rot{Grenz-\Lab h\Rab} (\textit{Ehe}, \textit{nahe}, \textit{rohe} usw.) ist kein Segment.
%     \item \rot{\Lab ß\Rab} ist besonders.
%   \end{enumerate}
% \end{frame}
...
%   \pause
%   \Zeile
%   Achtung: Da \textit{-in} einen Nebenakzent trägt, liegt in \textit{Schülerinnen} /ʃyləʁɪnən/ $\Rightarrow$ [ˈʃyː.lə.ˌʁɪṇən] und ähnlichen Wörtern ein Silbengelenk vor!
\end{frame}
