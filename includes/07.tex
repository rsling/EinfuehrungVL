...
\begin{frame}[fragile]
  ...
  \begin{center}
    ...
    \begin{tikzpicture}[scale=3.5,baseline=default]
      ...
      \only<9>{\path (uu)  edge [orange, *-, bend right=10] (I);}
  ...
  \begin{center}
    \resizebox{\textwidth}{!}{
      \begin{tabular}{llp{0mm}llp{1mm}lp{2mm}llp{2mm}l}
        ...
        & \textbf{Gen} && Stuhl\gruen{-es} & Haus\gruen{-(e)s} && Staat\gruen{-(e)s} && Frau\onslide<5->{\rot{*-s}} & \multicolumn{1}{l}{Sau\onslide<5->{\rot{*-s}}} && Auto\gruen{-s} \\
        \midrule
...
\begin{frame}
  ...
  \begin{block}{Artikel und Pronomen}
    Wenn die Formen in Artikelfunktion und Pronominalfunktion nicht durchgehend gleich sind, nehmen wir \textbf{zwei verschiedene lexikalische Wörter mit gleichlautendem Stamm} an: einen Artikel und ein Pronomen.
  \end{block}
\end{frame}
