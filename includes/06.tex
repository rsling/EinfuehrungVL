...
\begin{frame}
  ...
  \begin{exe}
    ...
    \ex
    \begin{xlist}
      \ex lauf-en (Inf) → \alert{lauf}\rot{-e} (1 Sg Prs Ind)
      \ex Münze (Sg) → \alert{Münze}\rot{-n} (Pl)
    \end{xlist}
  \end{exe}
  \pause
  \Halbzeile
...
\begin{frame}
  ...
  \begin{exe}
    \ex
    \begin{xlist}
      \ex{\alert{[Gewichtheberinnen]} haben \alert{[ein hartes Trainingsprogramm]}.}
      \pause
      \ex{\alert{[Trainierte Gewichtheberinnen]} haben \alert{[Chancen]}\\auf \alert{[die Goldmedaille]}.}
      \pause
      \ex{\alert{[Eine hervorragende Gewichtheberin]} wurde \alert{[Olympiasiegerin]}.}
    \end{xlist}
  \end{exe}
  \pause
...
\begin{frame}
  ...
  \begin{exe}
    \ex
    \begin{xlist}
      \ex{Wir sehen \rot<13->{den Rasen}.}
      \pause
      \ex{Wir begehen \rot<14->{den Rasen}.}
      \pause
      \ex{Wir säen \rot<15->{den Rasen}.}
      \pause
      \ex{Wir fürchten \rot<16->{uns}.}
    \end{xlist}
    \pause
    \ex
    \begin{xlist}
      \ex{Sarah backt \rot<17->{ihrer Freundin} einen Marmorkuchen.}
      \pause
      \ex{Wir kaufen \rot<18->{dir} ein Kilo Rohrzucker.}
      \pause
      \ex{Die Mannschaft spielt \rot<19->{mir} zu drucklos.}
      \pause
      \ex{Der Marmorkuchen schmeckt \rot<20->{den Freundinnen} gut.}
    \end{xlist}
    \pause
    \ex
    \begin{xlist}
      \ex \rot<21->{Nächsten März} fahre ich zum Bergwandern in die Tatra.
      \ex Es waren \rot<22->{den ganzen Tag} Menschen zum Gipfel unterwegs.
    \end{xlist}
    \pause
    \ex
    \begin{xlist}
      \ex Das Ferienhaus \rot<23->{einer Freundin} steht im Juni leer.
      \ex Der Kragen \rot<24->{der Jacke} \rot<25->{meiner Oma} ist dreckig.
    \end{xlist}
  \end{exe}
\end{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item Kongruenz:
      \begin{itemize}[<+->]
        ...
        \item \rot{beide Einheiten} haben das Markmal
    ...
    \item Rektion:
      \begin{itemize}[<+->]
        ...
        \item \rot{Regens} hat das regierte Merkmal \rot{nicht}
